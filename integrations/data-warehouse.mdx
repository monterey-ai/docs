...
1. request our Google Cloud service account details
2. add the service account email address to your Google Cloud IAM policy and grant it the `bigquery.jobUser` _(BigQuery Job User)_ role. This will allow us to run queries on your behalf.
3. share the dataset with our service account email address and grant it the `bigquery.dataViewer` _(BigQuery Data Viewer)_ role. This will allow us to access only the datasets you want to share.
4. share with us the dataset id and Google Cloud project id
