...
<Steps>
  <Step title='Click "Connect Monterey to Slack"'></Step>
...
To begin pulling conversations into Reforge Insight Analytics for analysis, you will need to select the channels you want to ingest.
...
Anyone who has the have the access to the specified Slack channel can create feedback from slack to Reforge Insight Analytics even when they do not have access. Only Reforge Insight Analytics teammates will be able to view feedback in Reforge Insight Analytics.
...
Once Reforge Insight Analytics and Slack are connected successfully and you have [selected channels for ingestion](/integrations/slack#feedback-ingestion), data will be pulled from periodically. The first time Reforge Insight Analytics loads data, it will look back 1 week. After that, it run every multiple times a day, up to hourly, depending on your subscription plan.

#### Data Collected

The following data attributes are loaded from Slack and utilized in Reforge Insight Analytics for analysis:
