...
- `domain` - The company's domain (e.g., "example.com"). For multiple domains, separate them with a semicolon (e.g., "example.com;example.net")
...
Acme Corp,acme.com;acmecorp.com,50000,Manufacturing,1000
...
1. **First**, looks for an existing account with matching `domain` in your workspace (matches against any of the domains if multiple are provided).
...
- Multiple domains should be separated by semicolons (e.g., "example.com;example.net").
...
3. **When using multiple domains:**
   - Separate them with semicolons
   - List the primary domain first
   - Ensure each domain is correctly formatted
