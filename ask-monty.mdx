...
The conversation is started entering by inputting an initial query. This query will be used to generate the dataset(s) that will supply the entire message chain.
...
- Themes: a query that is bested answered via clustering the data into a few key topics.
- Examples: a query that is best answered via listing example feedback.
- User Stories: a query that is best answered via a generated user story.
- PRDs: the a query that is best answered via a generated a draft product requirement document.
- Cohorts: a query that is best answered by created a cohort of customers all discussing the particular user query.
- Snippet Examples: a query that is best answer by creating and returning a list of relevant snippets to the user query.
...
<img width="100%" src="./images/ask_monty_grouped_bar.png" class="ask-monty-image" />
...
\*\_Metadata support is enabled for string-typed feedbcack, customer and account level metadata that is enabled in the metadata configuration settings.
