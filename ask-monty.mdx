...
Delve deep into your feedback with "Ask Monty", an advanced GPT-enabled analysis tool. Pose questions regarding your feedback data, and let Monty analyze, visualize and provide insightful answers.
...
The conversation is started by entering by inputting an initial query. This query will be used to generate the dataset(s) that will supply the entire message chain.
...
- **Be Specific**: The more specific your query, the more accurate the response will be. Avoid vague and short queries like "users mad" or "bugs". While Monty will do its best to interpret these queries and extract the relevant filters/snippets, the response will be more focused with a more specific query.
...
- **Project Labels**: "from the spelling bee game project", etc.
- **Tag Labels**: "from feedback tagged 'xyz'", "from feedback tagged 'abc'", etc.
...
- Metadata (for enabled categorical type metadata keys)

## Dataset Actions

### Datset Edits
Dataset filters can be edited, added, or removed at any time following the initial query extraction. 
