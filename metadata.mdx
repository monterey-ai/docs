...
There are 3 types of metadata we store and utilize in Monterey:

- Feedback-Level Metadata: metadata that is tied to a specific feedback item, such as the version of the app of an iOS review
- Customer-Level Metadata: metadata that tied to the customer providing feedback, such as a job title, username, or tracked user-event
- Account-Level Metadata: metadata that is tied to the overarching account of a group of customers, such as an account contract value

All 3 types of metadata are configurable and usable throughout the app.

# Configuration

Metadata is configurable in the metadata configuration page in Workspace Settings -> Metadata.

The first thing you will see is a list of all available metadata keys, split into separate tables by metadata type.
...
To edit any key's configuration, simply expand that keys row where you can then edit the name, description, type and enabled fields.
...
On the sidebar, you will see each of the 3 metadata types available as configured in the sidebar under "Account", "Customer metadata" and "Feedback metadata":
...
A great way to utilize customer and account-level metadata is when creating and viewing customer cohorts.
